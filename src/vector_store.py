...
from config import COMPANY_KNOWLEDGE_COLLECTION, PERSISTENT_DB_PATH
from document_manager import DocumentManager, generate_document_id, process_file_for_rag
        ...
        if action == "delete":
            try:
                ...
                if existing_docs["ids"]:
                    ...
                    print(
                        f"Deleted {len(existing_docs['ids'])} vectors for deleted file: {file_path}"
                    )
            ...
            try:
                ...
                if existing_docs["ids"]:
                    collection.delete(ids=existing_docs["ids"])
                    print(
                        f"Removed {len(existing_docs['ids'])} existing vectors for: {file_path}"
                    )
            except Exception as e:
                print(
                    f"Warning: Could not remove existing vectors for {file_path}: {e}"
                )
            ...
            if documents:
                ...
                collection.add(
                    documents=doc_contents, ids=doc_ids, metadatas=doc_metadatas
                )
